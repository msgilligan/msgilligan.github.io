...
//			if (window.iui_ext)	window.iui_ext.injectEventMethods(page);	// TG -- why was this comment left here??
			...
			if (currentDialog)
			{
            	iui.setShown(currentDialog, false);
//				currentDialog.removeAttribute("selected");
	...
	insertPages: function(frag)
	{
		...
		for (var i = 0; i < nodes.length; ++i)
		{
			...
			if (child.nodeType == 1)
			{
				...
				if (iui.getShown(child) || child.getAttribute("selected") == "true" || !targetPage)
					targetPage = child;
	...
	*/
	getSelectedPage: function()
	{
		...
		{
			if (child.nodeType == 1 && (iui.getShown(child) || child.getAttribute("selected") == "true"))
				return child;
		}	 
	},
	
	getShown: function(view)
	{
	    return view.getAttribute("shown") == "true";
	},

	setShown: function(view, isShown)
	{
	    if (isShown)
	    {
